...
function sidebar_effect_section(sgb, gene_object, col) {
  ...
  columns = columns.concat([{'column': col.column+'_half1'}, {'column': col.column+'_half2'}]);
...
class customHeatmapTrack extends heatmapTrack {
    ...
    if (gene_object) {
      ...
      let html = `
          <div class="gene_tooltip">
            ...
            <p><strong>(mean) T-value:</strong> ${gene_object[column.column+'_T'].toFixed(3)}</p>
          </div>
        `
  ...
  make_summary_sidebar(col) {
    ...
    const bottomGenes = sorted.slice(Math.max(ngenes-10, topGenes.length)).objects().map(row => ({ gene: row, score: row[col.column], T: row[col.column+'_T'] }));
    ...
    headerRow.selectAll('th')
      .data(['Name', 'Description', 'L2FC', 'T-value'])
      .enter()
      .append('th')
      .style('padding', '4px')
      .style('border-right', (d, i) => i < 2 ? '1px solid #ddd' : null)
    ...
    const createRows = (data, isTop) => {
      ...
      rows.append('td')
        ...
        .style('background-color', d => d3.scaleLinear()
          .domain([0, 6])
          .range(["#CCCCCC", "#FF0000"])(Math.abs(d.T)))
    ...
    function updateScatterplot() {
      ...
      if (half_half_plot) {
        ...
        plotData = self.data.data
          .select([xcol, ycol, xcol+'_num_filtered_insertions', ycol+'_num_filtered_insertions', self.contig_col, self.start_col, self.end_col, self.id_col, 'name', 'desc'])
          .rename({[xcol]: 'x', [ycol]: 'y', [xcol+'_num_filtered_insertions']: 'xf', [ycol+'_num_filtered_insertions']: 'yf'})
          .filter(d => d.x && d.y) // filtering out nulls
          .objects();
      }
...
async function load_browser(strain, contig, region, focal_col=null) {
  ...
  const tnseq_file = `../data/${strain}/RBTnSeq.feather`;
  const fna = `../data/${strain}/genome.fna`;
  const faa = `../data/${strain}/aaseq`;
  const gene_table_file = `../data/${strain}/genes_w_ecocyc.tab`;

  const metadata = await d3.csv('../data/RBTnseq_sets.csv');
  let example_time_zeros = {};
  metadata.forEach(d => {
    for (let col of d.time0.split(';')) {
      example_time_zeros[col.split('S')[0]] = col; // just keep overwriting, any time zero example is good enough
    }
  });
  ...
  const rbtnseq_data = await aq.loadArrow(tnseq_file);
  console.log('RBTnSeq data loaded:', rbtnseq_data);
  ...
  my_browser.loadingPromise.then(sgb_instance => {
    ...
    cht.filter_change_button = cht.add_controls_button('Filtering log2 fold-change data  : ON ',
      function() {
        cht.filter_by_strain_count = !cht.filter_by_strain_count;
        if (cht.filter_by_strain_count) {
          cht.filter_change_button.text('Filtering log2 fold-change data (≥3 barcodes with ≥5 counts): ON ');
          cht.filter_change_button.style('background-color', '#ccc');
          cht.load_region();
        } else {
          cht.filter_change_button.text('Filtering log2 fold-change data (≥3 barcodes with ≥5 counts): OFF (not recommended)');
          cht.filter_change_button.style('background-color', '#faa'); 
          cht.load_region();
        }
      }
    );
    ...
    const subtrack_data = {}
    subtrack_data['count'] = new serverPointData(sgb_instance, 'tnseq_count_data', fetch_path, {strain_name: strain, file_type: 'counts'}, {contig_col: 'scaffold'});
    subtrack_data['count'].base_columns = subtrack_data['count'].base_columns.concat([{name: 'barcode', dtype: 'str'}, {name: 'strand', dtype: 'str'}]); // need to fetch for coloring
    subtrack_data['fit'] = new serverPointData(sgb_instance, 'tnseq_fit_data', fetch_path, {strain_name: strain, file_type: 'fits'}, {contig_col: 'scaffold'});
    subtrack_data['fit'].base_columns = subtrack_data['fit'].base_columns.concat([{name: 'barcode', dtype: 'str'}, {name: 'strand', dtype: 'str'}]); // need to fetch for coloring
    ...
    const load_subtracks = function(which) {
      ...
      for (let c of cols) {
        cht.subtracks[which].push(new serverTnSeqPointTrack(sgb_instance, `Barcode ${whichname}: ${c.name}, ${c.column}`, c.column, {title: '', y_scale_type: scaletype}, 'scaffold', 'pos', `tnseq_${which}_data`))
      }
      if (which=='count') {
        const t0_ex = example_time_zeros[cols[0].column.split('S')[0]];
        cht.subtracks[which].push(new serverTnSeqPointTrack(sgb_instance, `Example time zero ${t0_ex}`, t0_ex, {title: '', y_scale_type: scaletype}, 'scaffold', 'pos', `tnseq_${which}_data`))
      }
    ...
    cht.count_col_button = cht.add_controls_button('Hide barcode counts for focal column',
      function() {
        if (cht.subtracks_on['count']) {
          cht.count_col_button.text('Show barcode counts for focal column')
          cht.count_col_button.style('background-color', '#ccc'); 
          cht.subtracks_on['count'] = false;
          reset_subtracks('count');
        } else {
          cht.count_col_button.text('Hide barcode counts for focal column')
    ...
    cht.fit_col_button = cht.add_controls_button('Show barcode log2 fold-change for focal column',
      function() {
        if (cht.subtracks_on['fit']) {
          cht.fit_col_button.text('Show barcode log2 fold-change for focal column')
          cht.fit_col_button.style('background-color', '#ccc');
          cht.subtracks_on['fit'] = false;
          reset_subtracks('fit');
        } else {
          cht.fit_col_button.text('Hide barcode log2 fold-change for focal column')
    ...
    sgb_instance.tracks[1].loadingPromise.then(tnseq_track => {
      sgb_instance.tracks.push(new serverTnSeqPointTrack(sgb_instance, 'tnseq_counts', 280, 685, 
        {'yticks': [5, 100, 1000], 'ytick_formatter': (d) => d == 1000 ? '1k+' : d == 5 ? '<=5' : d, 'clip': true, 'log_y': true, 'load_threshold': 100000}, 
        'scaffold', 'pos', strain, 'int', 'counts', tnseq_track));
      sgb_instance.tracks.push(new serverTnSeqPointTrack(sgb_instance, 'tnseq_fits', 280, 925, {'yticks': [-6,-4,-2,0,2,4,6], 'clip': true, 'load_threshold': 100000}, 'scaffold', 'pos', strain, 'float16', 'fits', tnseq_track));
      tnseq_track.row_name_click = () => {
        sgb_instance.tracks[2].display_region();
        sgb_instance.tracks[3].display_region();
      };
      sgb_instance.tracks[2].color_func = coloring_by_strand;
      sgb_instance.tracks[3].color_func = coloring_points;
    });
    */
  });
}
